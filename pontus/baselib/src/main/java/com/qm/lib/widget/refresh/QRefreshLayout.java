...
public class QRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    private boolean isRefreshEnable; // 是否可以加载更多
    ...
    public QRefreshLayout(Context context, AttributeSet attrs) {
        ...
        viewLoadContainer.setBackgroundColor(getResources().getColor(R.color.white));
    ...
    public void setRefreshEnable(boolean isEnable) {
        ...
        if (isRefreshEnable) {
            viewRefreshContainer.setVisibility(VISIBLE);
            setPullToRefreshHeight(170);
        } else {
            viewRefreshContainer.setVisibility(GONE);
            setPullToRefreshHeight(20000);
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (!isEnabled() || isAnimating || isNestedScrolling || isInSecondFloor || !isRefreshEnable) {
            return false;
        }
...
//        KLog.printTagLuo("canChildScrollDown()... " + viewTarget.canScrollVertically(1));
...
//        KLog.printTagLuo("canChildScrollUp()... " + viewTarget.canScrollVertically(1));
        return viewTarget.canScrollVertically(1);
    }
