...
import '../model/home/home_bean_image.dart';
import '../utils/route_util.dart';

class HomeBanner extends StatefulWidget {
  final _homeBannerHeight;

  final List<HomeBeanImage> topList;
...
class _HomeBannerState extends State<HomeBanner> {
  ...
  List<HomeBeanImage> _fakeList = [];

  Duration _bannerDuration = new Duration(seconds: 3);

  Duration _bannerAnimationDuration = new Duration(milliseconds: 300);
  ...
  Widget _buildItem(BuildContext context, int index) {
    HomeBeanImage item = _fakeList[index];
    return GestureDetector(
      child: GestureDetector(
        onTap: () {
          RouteUtil.route2Web(context, "", item.linkUrl);
        },
        onTapDown: (donw) {
          _isEndScroll = false;
        },
